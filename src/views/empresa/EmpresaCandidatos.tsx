    ...
    <div className="space-y-4">
              ...
              {filtered.map((row) => (
                <tr key={row.c.id} className="table-row cursor-pointer" onClick={() => setSelected(row.c)}>
                  <td className="table-td">
                    <p className="font-medium text-slate-900">{row.c.nombre}</p>
                    <p className="text-[12px] text-slate-400">
                      {row.c.ciudad} · {row.c.experiencia} años de experiencia
                    </p>
                  </td>
                  <td className="table-td text-slate-600">{row.oferta}</td>
                  <td className="table-td whitespace-nowrap font-mono text-[12.5px] text-slate-500">{row.fecha}</td>
                  <td className="table-td">
                    {row.estado === 'Favorito' ? (
      ...
      {selected && (
        <div className="fixed inset-0 z-50">
          <div className="absolute inset-0 bg-slate-900/25" onClick={() => setSelected(null)} />
          <aside className="animate-slide-in absolute inset-y-0 right-0 flex w-full max-w-sm flex-col border-l border-line bg-white shadow-pop">
            <div className="flex shrink-0 items-start justify-between gap-3 border-b border-line px-5 py-4">
              <div className="min-w-0">
                <p className="truncate text-[15px] font-semibold text-slate-900">{selected.nombre}</p>
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="border-b border-line px-5 py-3">
                ...
                <p className="mt-2.5 text-[13px] leading-relaxed text-slate-600">{selected.resumen}</p>
              </div>
              ...
              <div className="border-t border-line px-5 py-3">
                ...
                <div className="flex items-center gap-2 py-1">
                  <span className="flex-1 font-mono text-[12px] text-slate-600">CV.pdf</span>
                  <button
